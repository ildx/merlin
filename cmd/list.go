...
var listCmd = &cobra.Command{
	...
	Long: `List Homebrew packages, Mac App Store apps, configuration tools, or profiles.

BEHAVIOR
	Without a subcommand shows an overview of brew, mas, and configs.
...
TIPS
	Use with grep for quick filtering: merlin list brew | grep docker
	Combine with 'merlin link --profile' for selective linking.`,
...
var listProfilesCmd = &cobra.Command{
	...
	Long:  "List all configuration profiles from root merlin.toml",
	Run: func(cmd *cobra.Command, args []string) {
		if err := runListProfiles(); err != nil {
			fmt.Fprintf(os.Stderr, "Error: %v\n", err)
			os.Exit(1)
		}
	},
...
func runListProfiles() error {
	...
	// Parse root merlin.toml
	rootPath := repo.GetRootMerlinConfig()
	rootConfig, err := parser.ParseRootMerlinTOML(rootPath)
	if err != nil {
		return fmt.Errorf("failed to parse root merlin.toml: %w", err)
	}

	if len(rootConfig.Profiles) == 0 {
		fmt.Println("\nNo profiles defined in merlin.toml")
		return nil
	}
	...
	fmt.Printf("Repository: %s\n\n", repo.Root)
	fmt.Printf("Found %d profile(s)\n", len(rootConfig.Profiles))
	fmt.Println(strings.Repeat("─", 80))

	// Get current hostname for auto-detect indicator
	sysInfo, _ := system.GetSystemInfo()
	currentHostname := sysInfo.Hostname

	// Print each profile
	for _, profile := range rootConfig.Profiles {
		// Profile name with indicators
		nameStr := profile.Name
		if profile.Default {
			nameStr += " (default)"
		}
		if profile.Hostname == currentHostname {
			nameStr += " [auto-detected for this machine]"
		}
		...
		if len(profile.Tools) > 0 {
			fmt.Printf("   Tools (%d): %s\n", len(profile.Tools), strings.Join(profile.Tools, ", "))
		} else {
			fmt.Printf("   Tools: (none specified - will use all)\n")
		}
	}
