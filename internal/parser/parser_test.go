...
`
		...
		// Verify backward compatible plain string decoding maps to ScriptItem.File
		if config.Scripts.Scripts[0].File != "install_extensions.sh" {
			t.Errorf("expected script file install_extensions.sh, got %s", config.Scripts.Scripts[0].File)
		}
	})

	// New: tagged scripts decoding
	t.Run("tool with tagged scripts", func(t *testing.T) {
		content := `
[tool]
name = "example"
...
scripts = [
  "plain.sh",
  { file = "tagged.sh", tags = ["full", "slow"] },
  { name = "altname.sh", tags = ["dev"] }
]
`
		path := createTestFile(t, content)
		defer os.Remove(path)
		...
		if len(config.Scripts.Scripts) != 3 {
			t.Errorf("expected 3 scripts, got %d", len(config.Scripts.Scripts))
		}

		// Plain script
		if config.Scripts.Scripts[0].File != "plain.sh" || len(config.Scripts.Scripts[0].Tags) != 0 {
			t.Errorf("expected plain.sh with 0 tags, got %s (%d tags)", config.Scripts.Scripts[0].File, len(config.Scripts.Scripts[0].Tags))
		}

		// Tagged script
		if config.Scripts.Scripts[1].File != "tagged.sh" || len(config.Scripts.Scripts[1].Tags) != 2 {
			t.Errorf("expected tagged.sh with 2 tags, got %s (%d tags)", config.Scripts.Scripts[1].File, len(config.Scripts.Scripts[1].Tags))
		}
		foundSlow := false
		for _, tg := range config.Scripts.Scripts[1].Tags {
			if tg == "slow" {
				foundSlow = true
			}
		}
		if !foundSlow {
			t.Error("expected 'slow' tag on tagged.sh")
		}

		// Alternate key 'name'
		if config.Scripts.Scripts[2].File != "altname.sh" || len(config.Scripts.Scripts[2].Tags) != 1 {
